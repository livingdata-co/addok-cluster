...
export async function startRedisServer(basePath, startupTimeout) {
  const fullPath = path.resolve(basePath)
  const dumpFilePath = path.join(fullPath, 'dump.rdb')

  try {
    await access(dumpFilePath)
  } catch {
    throw new Error(`No Redis dump file found at ${dumpFilePath}`)
  }

  const instance = await createInstance(basePath, {crashOnFailure: true, startupTimeout})
  return [{socketPath: instance.socketPath}]
}
